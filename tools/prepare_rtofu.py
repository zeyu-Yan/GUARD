...
def normalize_row(row: dict[str, Any], idx: int, *, task_id: str, split: str) -> dict[str, Any]:
    ...
    for key in ("source_id", "category", "author", "book", "source_dataset"):
        if key in row:
            record[key] = row[key]
    return record
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Normalize r-tofu forget/retain splits into GUARD JSON files.")
    parser.add_argument("--forget", type=Path, required=True)
    parser.add_argument("--retain", type=Path, required=True)
    parser.add_argument("--out_dir", type=Path, default=GUARD_ROOT / "data" / "rtofu")
    ...
    forget_rows = maybe_limit(read_json_or_jsonl(args.forget), int(args.forget_limit), int(args.seed), args.sample)
    retain_rows = maybe_limit(read_json_or_jsonl(args.retain), int(args.retain_limit), int(args.seed), args.sample)
    forget_out = [
        normalize_row(row, idx, task_id=str(args.task_id), split="forget")
        for idx, row in enumerate(forget_rows)
    ]
    retain_out = [
        normalize_row(row, idx, task_id=str(args.retain_task_id), split="retain")
